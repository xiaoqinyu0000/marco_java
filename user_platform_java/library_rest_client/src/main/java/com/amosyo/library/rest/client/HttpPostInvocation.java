...
public class HttpPostInvocation implements Invocation {
    ...
    public HttpPostInvocation(@NonNull final URI uri, @NonNull final Entity<?> entity, @NonNull final Map<String, Object> headerMap) {
        this.mUri = requireNonNull(uri, "uri");
        this.mHeaderMap = requireNonNull(headerMap, "headerMap");
        this.mEntity = requireNonNull(entity, "entity");
    }
    ...
    public Response invoke() {
        ...
        final RequestBody requestBody = RequestBody.create(mEntity.getEntity().toString().getBytes(StandardCharsets.UTF_8));
